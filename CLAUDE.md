...
- [x] **Portal Monday fallback hardening (30 Apr 2026):** Two fixes shipped together to stop the `[PORTAL FALLBACK]` alert email firing for non-bugs.
  - **`PORTAL_MONDAY_FALLBACK_ENABLED=false` flipped on Netlify production.** Monday is no longer queried by any of the 13 fallback-wired portal routes. When OP errors, the user sees a clean 502 instead of stale Monday data. The flag still exists for emergency rollback.
  - **`OpApiError` + `isOpClientError` introduced in `src/lib/op-api.ts`.** `opFetch` and `opPostJson` now throw a typed error carrying the HTTP status. Routes' catch blocks branch: `isOpClientError(err)` → propagate the 4xx to the user as-is (no `reportFallback`, no Monday call, no alert). Only 5xx / network errors fire the fallback chain. Stops false-positive alerts on legitimate "job not found / not assigned to you", "session expired", "verification code wrong", etc. Wired into 11 routes — `/api/jobs`, `/api/jobs/[id]`, `/api/jobs/[id]/complete`, `/api/jobs/run`, `/api/auth/forgot-password`, `/api/auth/reset-password`, `/api/auth/verify-reset-token`, `/api/auth/register/start|verify|complete`, `/api/settings/notifications` (GET + POST). Login + bookout-token deliberately untouched (already shaped correctly: login uses structured response not throw; bookout-token's reportFallback only fires on `!isOpMode()` which is unreachable today).
  - **Convention for new portal routes:** any new route that wraps an OP call MUST use this catch-block shape, otherwise legitimate 4xx responses will fire the alert email.
- [x] **Staff settings repointed off Monday (30 Apr 2026):** The freelancer portal's staff Crew & Transport calculator (`/staff/crew-transport`) used to read its rates from a Monday "D&C Settings" board (id `18398014955`). That board is now stale — OP's `calculator_settings` table is canonical (same source the OP-side calculator uses). New OP endpoint `GET /api/portal/staff/calculator-settings` (in `backend/src/routes/portal.ts`, mounted before `router.use(portalAuth)`) returns the values shaped to match the portal's `CostingSettings` interface, protected by header `x-portal-staff-key` matching env `PORTAL_STAFF_API_KEY`. Portal's `src/app/api/staff/settings/route.ts` rewritten to call this endpoint with the shared key; falls through to in-file defaults if OP unreachable so the staff page stays usable. Monday GraphQL call gone, board ID + column IDs removed from the file. New env var on **both** OP server (`/var/www/ooosh-portal/backend/.env`) and Netlify portal: `PORTAL_STAFF_API_KEY=<shared secret>`. Map of `calculator_settings` keys → portal field names is in the OP route handler — extend if a new rate is added.
- [ ] **Cleanup follow-ups (after a couple of weeks running on `PORTAL_MONDAY_FALLBACK_ENABLED=false`):**
  - Delete `src/lib/monday.ts` + post-OP-call Monday code in the 13 portal routes
  - Retire the Monday "D&C Settings" board (no longer read by anything)
