...
*Immediate follow-on after PR 2 (NEXT BUILD — handoff written):* freelancer-led interim check-in UI. ~90% of the machinery already exists (soft check-in primitive, interim PDF variant, freelancer-bookout JWT flow). Full step-by-step handoff in **`docs/VAN-SWAP-AND-SOFT-CHECKIN-SPEC.md` §11** — what exists, what to build (widen the freelancer save-event guard at `vehicles.ts` ~2016 to allow `soft-check-in`, build the freelancer entry point + interim form, fire the interim PDF), and the open questions to settle with jon first.

*Post-PR-2 follow-up fix (shipped May 2026):* excess **move-not-copy** + exclude `swapped` from the self-drive van count (`money.ts`) and dispatch-check (`assignments.ts`). The swap was copying the held `job_excess` record to the new assignment (leaving the original on the `swapped` row), which would have double-counted on the Money tab (£2400 vs £1200) — caught by the read-only preview script (`scripts/preview-vehicle-swap.ts`) on a real 2-driver hire before any live swap ran. Swap now re-points `assignment_id` (one row follows the hire).
...
- [x] **Freelancer book-out Round 4 — hire-form integration + smart resolve + email fallback (30 Apr 2026):**
  ...
  - **Still TODO (Round 5+):** Vehicle swap mid-hire (Phase D3 — UI shipped May 2026, see `docs/VAN-SWAP-AND-SOFT-CHECKIN-SPEC.md`). Multi-van D&C (drivers × vans expansion of the resolve merge). **Freelancer-led interim check-in — NEXT BUILD, handoff in spec §11** (reuses the soft check-in primitive + freelancer-bookout JWT flow; widen the freelancer save-event guard to allow `soft-check-in`, build the freelancer entry point + interim form + interim PDF).
