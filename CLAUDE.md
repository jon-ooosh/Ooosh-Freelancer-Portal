...
- **Automation loop** (§9-§10, migration 165 master switch, 166 default sender) — the reply-bump + suppression gate + scheduled draft/send:
  ...
  - **Runner** (`services/auto-chase-runner.ts`, cron 08:10 Europe/London + `POST /api/auto-chase/run-due`): due jobs with `auto_chase_mode IN ('draft','send')` → gate → Gmail draft (draft) or send (send). **`auto_chase_send_enabled` master switch defaults OFF** → a 'send' job only drafts until flipped (graduation). Every action logs a `source='system'` chase interaction; failures/suppressions defer the chase. Sign-off: job's manager1 first name → `chase_default_sender_name` setting → "the Ooosh team" (manual "Draft chase" button uses the clicker's name).
...
- Phase 4: the §7.2 dispute helper (NL query over the ingested chain) + line-item diff history.
