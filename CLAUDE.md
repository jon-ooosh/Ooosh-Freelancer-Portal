...
- ~~**Vehicle condition report PDF failure (C3, 22 Apr 2026)**~~ — **RESOLVED 22 Apr 2026.** Root cause was `drawText` drawing `✓` (U+2713) with `StandardFonts.Helvetica` (WinAnsi), which throws `WinAnsi cannot encode "✓" (0x2713)`. Same class of bug would fire for em-dashes, curly quotes, ellipses or any non-WinAnsi char in briefing items or notes. Fix: `/api/vehicles/generate-pdf` (and a new shared `buildConditionReportPdf` helper) now embed Roboto via fontkit with a StandardFonts fallback, using `backend/src/services/pdf-fonts.ts`. `package.json` build script now copies the Roboto `.ttf` files into `dist/services/fonts/` so prod actually finds them (prior state: `tsc` alone → `[hire-form-pdf] Using fallback StandardFonts` in prod logs). Also added `POST /api/vehicles/events/:eventId/regenerate-pdf` for mis-fire backfills + damage-dispute re-sends, surfaced in the Event History section of VehicleDetailPage. Going forward, `save-event` persists signature base64 as a separate R2 png (`vehicle-events/{REG}/{id}_signature.png`) and `briefingItems` on the event JSON so regenerations have full fidelity.
