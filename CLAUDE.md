...
- [x] **Slice 3 (staff cockpit — shipped before slice 2):** `CarnetSection` self-fetching card on Job Detail Overview (hides when no carnet). Lifecycle stepper (warnings-not-gates), custody control (ooosh/client/issuer auto-set on status), manual detail edit, GMR management (add/edit/status/QR upload/mark-sent/delete), document attachments, soft cancel. Backend write endpoints on `/api/carnets` (POST create, PATCH with status→custody/timestamp side-effects + timeline interaction, cancel, GMR CRUD, files JSONB).
