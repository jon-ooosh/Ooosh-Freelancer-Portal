...
- [x] **payment-event stale `excess_id` silent no-op (FIXED Jul 2026, job 16085):** the portal passes an `excess_id` (from Stripe checkout metadata, captured at session creation) back on the excess payment-event, and OP trusted it blindly — `UPDATE job_excess ... WHERE id = $excess_id` matching 0 rows (stale UUID, or a record on a different job for the same client) threw no error, returned 200, and the money never landed on the record. Worse, the `job_payments` audit row (carrying `hh_deposit_id`) then suppressed BOTH the Money-tab "unmatched HH deposit" banner AND the passive reconciliation's already-linked check, so the state could never self-heal. THREE fixes in `money.ts`: (a) payment-event validates a supplied `excess_id` exists AND belongs to the job, else falls back to the find-or-create path (+ rowCount error log on the charge UPDATE); (b) the already-linked checks (reconciliation + unmatched banner) only count `job_payments` excess rows whose money actually landed (`JOIN job_excess ... AND (taken > 0 OR held > 0)`); (c) `reconcileExcessDeposits` matchable statuses widened to include **`pending`** — the status every hire-form-created record sits in awaiting payment — which the safety net had been skipping entirely (only `needed`/`partially_paid` matched). Reconciliation now also fires `syncExcessRequirementStatus` when it applies money so the requirement card/gate agree. Post-fix, affected jobs self-heal on Money-tab load or the nightly 03:00 job-financials pass (look for `hh_reconcile_source='auto_match'`).
