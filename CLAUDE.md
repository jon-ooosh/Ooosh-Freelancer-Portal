...
- **Backup lever if 535s persist:** switch the transporter to a **pooled connection** (`nodemailer.createTransport({ pool: true, maxConnections, maxMessages })`) so connections are reused and we re-authenticate far less often — fewer fresh auths = fewer 535s. Not built (retry was the minimal fix); the docs already flag the transporter-singleton gotcha, so pooling needs care. Reach for this only if the retry policy proves insufficient in the logs.
