...
**Backline operations page — filtering + "where is it?" location (Jul 2026, `routes/backline.ts` + `BacklinePage.tsx`):**
- **"Coming Back" measures DE-PREP, not prep.** The default "Outstanding" filter and the "Done" pill are **direction-aware** (`matchesStatusFilter(job, 'prep'|'deprep')`): Going Out uses `effectivelyDone` (prep card done OR HH prepped/dispatched); Coming Back uses the **`post_hire` de-prep card's own status** (`backlineStatus === 'done'`). Pre-Jul 2026, "Outstanding" keyed off `effectivelyDone` for both — and a returning job is ALWAYS HH-dispatched+, so `effectivelyDone` was always true → the ENTIRE Coming Back list vanished under the default filter while the summary card still showed a count. Any new count/filter over these lists must respect the prep-vs-deprep split.
- The returning queries **prefer the post_hire card** via `DISTINCT ON (j.id) ... ORDER BY j.id, CASE WHEN jr.phase='post_hire' THEN 0 ELSE 1 END`. A plain `ORDER BY jr.phase DESC` picks `pre_hire` (alphabetical quirk: `'pre' > 'post'`) — that was the latent bug reinforcing the above. Don't revert to `phase DESC`.
- **Going Out windows + displays on the OUTGOING date** (`COALESCE(out_date, job_date)`), not job start. Coming Back windows on `return_date`.
- **Departure-date gate on Coming Back:** the returning queries also require `COALESCE(j.out_date, j.job_date)::date <= CURRENT_DATE`. Warehouse checks items out in HH the day BEFORE a van leaves, so a not-yet-departed job is already HH-5 while OP holds `prepped` — that HH-5 alone wrongly pulled it into Coming Back (incident: City & Moon 16278, going out tomorrow, showed as Coming Back → Done off the pre-hire prep card). The gate keeps a not-yet-left job in Going Out where it belongs. Do NOT "fix" this by gating on `pipeline_status='dispatched'` alone — a genuinely-out job whose staff never clicked Mark-as-Dispatched sits at HH-5 + OP-`prepped` (webhook maps HH 5 → OP `prepped`), indistinguishable by status; the departure DATE is the reliable discriminator.
- **"Where is it?" location** (migration 161, `backline_locations`, one row per requirement): records where prepped kit sits — `van` (+reg), `loading_bay`, `rehearsal`, `other` (free text). **Pre-hire only** (kit just returns to stock on the way back — no return-side prompt). Editable anytime the card is at least `in_progress` (PUT `/backline/location/:reqId` rejects `not_started` — can't stage a to-do). Reg is free text (queryable, not an FK) so client/sub-hire vans work; prefilled from vans allocated to the job (dual-match `job_id`/`hirehop_job_id`). Marking a backline card Done offers the modal (skippable). Shared `BacklineLocationModal.tsx` on both the Backline page rows and the Job Detail backline `RequirementCard`. Endpoints: `GET /backline/location-context/:reqId` (current + allocated regs + fleet suggestions), `PUT`, `DELETE`. Not surfaced elsewhere yet — the reg index future-proofs a "what's in this van" query.
