...
- **Photo clickability — photos in private bucket (23 Apr 2026)** — The condition report PDF wires up `textWithLink` "View full size" hyperlinks per photo using `r2Url`, built from `R2_PUBLIC_URL` (backend) or `VITE_R2_PUBLIC_URL` (frontend). BUT: `/upload-photo` in `vehicles.ts` writes to the default `ooosh-operations` R2 bucket (private) rather than the public `ooosh-vehicle-photos` bucket. So the PDF links are correctly embedded — clicking them opens the URL in a browser — but they'll 403 because the bucket isn't publicly readable. Fix: either (a) route `/upload-photo` through a second R2 client pointing at `ooosh-vehicle-photos` with a dedicated env-var set (cleanest, matches original design), or (b) use presigned URLs with a 7-day expiry (works short-term, breaks for historical damage disputes), or (c) add an authenticated `GET /api/vehicles/photo/:key` proxy endpoint (works for staff viewing, still 403 for clients). Non-blocking for the PDF generation itself.
