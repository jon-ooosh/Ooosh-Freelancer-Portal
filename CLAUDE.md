...
- [ ] **Backfill `xero_contact_id` + `client_name` on derivation-created `job_excess` records** — proper companion to the 060 display fix. Currently `hh-requirement-derivation.ts` inserts `job_excess` rows with both fields NULL, so they all collapse into the UNLINKED group on the ledger. Needs: (a) decide source of truth for `xero_contact_id` — likely a new `xero_contact_id` column on `organisations`, populated from HH contact sync, then read into the excess record at creation time; (b) at minimum, set `client_name` from `jobs.client_name` on auto-create so the existing detail page is still meaningful; (c) one-shot backfill script for the existing ~126 records, reading the linked job's client and stamping retroactively. Until done, the UNLINKED bucket on `/money/excess` will keep growing.
...
- [x] **Freelancer book-out token repoint (23 Apr 2026, foundation; deployed 24 Apr 2026)**: Portal's `/api/jobs/:id/bookout-token` now mints an HMAC token in OP format (`{expiry}.op.{quoteId}.{email}.{sig}`) and redirects to OP's `/vehicles/book-out` when `DATA_BACKEND=op`. OP exposes `POST /api/vehicles/freelancer-bookout/resolve` (public, HMAC-authenticated) which verifies the token, confirms the freelancer is on a `quote_assignments` row for the quote, locates the staff-allocated `vehicle_hire_assignments` row, and mints a narrow-scoped `freelancer_bookout` session JWT (4h TTL, scoped to one assignment). New middleware `authenticateFreelancerBookout` validates that JWT on protected routes. The embedded VM's existing `exchangeFreelancerToken` (in `useAuth.tsx`) was repointed from the legacy `/validate-freelancer-token` Netlify function to the new OP endpoint; returns `hhJobNumber` as `jobId` so downstream `BookOutPage` flow (which expects an integer) keeps working. Monday `VEHICLE_APP_URL` redirect preserved behind `PORTAL_MONDAY_FALLBACK_ENABLED` for the transition window.

  **Migration 062 deploy note (24 Apr 2026):** First deploy attempt failed because the run_groups backfill INSERT filtered out soft-deleted quotes (`AND q.is_deleted = false`), but the FK constraint added immediately after applies to every row in `quotes` regardless of `is_deleted`. A soft-deleted quote with a populated `run_group` UUID had no parent row, so `ALTER TABLE ADD CONSTRAINT quotes_run_group_fkey` raised 23503. The whole migration runs in a single `BEGIN/COMMIT` so production rolled back cleanly with nothing partially applied. Fix shipped in commit `a7a0065` — drop the `is_deleted` filter, every UUID gets a parent row regardless of soft-delete status. Migration ran clean on second attempt.

  **Status: production-deployed, foundation only.** The HMAC token redemption + session JWT minting is live, but the freelancer book-out flow is NOT yet usable end-to-end — see "Still TODO (next session)" below.
  ...
  **Still TODO (next session):**
  1. **Confirm the D&C allocation flow end-to-end** — staff allocates a van to a quote, freelancer clicks "Start delivery" in portal, lands on OP's BookOutPage with the right vehicle pre-selected. Will surface (a) whether the resolve query picks the correct `vehicle_hire_assignments` row in the real allocation flow, and (b) whether jon needs to firm up the freelancer→vehicle linkage at the data model level before this can be reliable for multi-crew jobs.
  2. **Wire the freelancer session JWT into the vehicle event endpoints** — `POST /api/vehicles/events`, photo upload (`POST /api/vehicles/upload-photo`), signature submission, fleet status update. Currently they're staff-JWT-only via `router.use(authenticate)` at the top of `vehicles.ts`. Either (a) accept both staff JWT and freelancer book-out JWT via a flexible auth middleware, OR (b) duplicate the route handlers under `/freelancer-bookout/*` with the narrower middleware. Option (a) is less code; option (b) is more explicit about scope. Prefer (a) but check with jon.
  3. **Update BookOutPage to use the assignment_id from the resolve response** — the resolve endpoint already returns `assignment.id`, `assignment.vehicleId`, `assignment.registration`, `job.hhJobNumber`. Currently `useAuth.tsx` only stashes `jobId` and `driverEmail` on `freelancerContext`. Extend `FreelancerContext` to also carry `assignmentId` + `vehicleId` + `vehicleReg`, and have `BookOutPage` skip the "Select Vehicle" step (step 0) entirely when in freelancer mode. Step 1 (Driver & Hire) auto-populates from the resolve response too. This makes the UX match the spec: freelancer lands directly on the walkaround.
  4. **Soft check-in (handover)** — deferred per jon. New event type capturing mileage + fuel + photos + signature when a freelancer picks up a van mid-tour (e.g. at Heathrow). Does NOT flip assignment status — the proper check-in at base still happens. Creates a baseline so damage/fuel from that point on is attributable. Spec is in the conversation history; needs writing up properly when we pick it up.
  5. **Remove Monday fallback in portal book-out path** — once 1-3 above are confirmed working in production for 1-2 weeks, delete the legacy Monday branch from `bookout-token/route.ts` (the `if (!mondayFallbackAllowed())` block + everything after) and remove `VEHICLE_APP_URL` from Netlify env.
...
- **Allocations booked-out UX (22 Apr 2026)** — Once a van is booked out (`rawStatus='booked_out'`), the allocation card should hide the "Book Out (not prepped)" button and show a "Booked Out" badge + "Mark as Returned" action. Currently the button still renders (disabled via `hire_status != 'Available'` only) and the Unlink Van button my earlier fix added excludes booked_out state. Combined with the "Allocations recognise booked-out state" item below, this needs a coordinated pass on AllocationsPage render logic using `rawStatus`.
...
- **Allocations: recognise booked-out state** — AllocationsPage still offers "Book Out" on assignments whose status is already `booked_out`/`active`, and those don't appear in the "Due Back" list either. The compat layer now exposes `rawStatus` (22 Apr fix) so the UI can branch on it — Book Out link should hide + a "Booked Out" badge show on booked-out cards, and the Due Back panel should include assignments with rawStatus in (booked_out, active). Defer to a dedicated session — the AllocationsPage needs a broader rethink anyway (van-centric rebuild above).
