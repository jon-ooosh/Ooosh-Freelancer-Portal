...
- **Per-job conversation SUMMARY (jon's chosen NEXT piece — start here in the new chat).** Spec §7.1/§7.2. An AI digest of the ingested email thread(s) on a job, shown at the top of the Activity Timeline / Money tab, cached + refreshed when new mail lands. Complements the collapse (collapse hides detail, summary surfaces the gist). Not built. Build a per-job summary first; the §7.2 dispute helper (NL query over the chain) is the richer Phase-4 follow-on.
- **Voice-learning from real examples (jon's request, Jul 2026).** jon wants to tune the chase voice by *showing example client replies + our actual replies* so the AI "learns" Ooosh's tone/style. Approach to design next session: let staff paste (or pick from ingested interactions) a few exemplar in/out pairs → distil them into the `chase_voice_instructions` setting (or a new few-shot examples setting appended to the prompt). Could be a "learn from this thread" affordance on the timeline, or a paste box in the chase-voice Settings section. The `chase_voice_instructions` knob already exists as the delivery mechanism; this is about *generating* good guidance from examples rather than jon hand-writing it. **The passive draft-vs-sent diff capture (§9.3) is the automated sibling** of this — capture what staff edited a draft into before sending, learn the patterns over time.
- Automatic 08:00 chase-due trigger for `auto_chase_mode='draft'` jobs + ChaseModal Off/Draft/Send toggle (makes drafting hands-off; `auto_chase_mode` column already exists).
...
- **If 535s STILL persist after pooling, or for durability, move off password-SMTP:** either the **Gmail API** (we already have service-account domain-wide-delegation infra in `config/gmail.ts` for the auto-chase inbox — add a `gmail.send` scope + impersonate `info@`; sends as the real @oooshtours.co.uk mailbox, no SMTP concurrency ceiling) or a **transactional provider like Resend/Postmark** (best-in-class deliverability + retries/webhooks, needs oooshtours.co.uk SPF/DKIM DNS setup + a vendor key). Decision deferred — pooling + retry is expected to resolve the observed pattern at current volume.
