...
- **Cancellation replacement-finder ("fill the gap")** — when a job cancels, surface other paused/open enquiries that could now take the freed slot. SQL match first (date overlap × resource match × not-already-confirmed), then optionally pass top N candidates to Claude for ranked rationale + draft re-engagement emails. Weighting heuristics jon flagged: longest-tour-fits-first (longer hires are more valuable per slot), bundle opportunities (van + backline together), surface potential job value alongside each candidate. Bucket priority: paused enquiries first, then open enquiries. Skip provisional (provisional ≈ confirmed, just waiting on £). Don't auto-email — surface candidates, staff triggers sends. Trigger surface = a new `/operations/fill-cancellation/:jobId` page (or panel within the existing Cancellations / Lost & Cancelled page), linked from the cancelled job banner. Use Claude API (key being loaded — first AI-integrated feature in OP, env var `ANTHROPIC_API_KEY` to be added when ready to wire up). Phased: (1) SQL matcher + UI panel with no AI; (2) AI rationale + draft emails on top; (3) proactive dashboard widget aggregating freed slots across recent cancellations. Caching for AI: prompt-cache the system prompt + cancelled-job context across all candidate ranking calls per cancellation event. **Recovery tracking (nice-to-have, not blocking):** if doable cleanly, add `replaced_cancelled_job_id UUID` on `jobs` so a confirmed booking can be linked back to the cancellation that freed the slot — set when staff sends a re-engagement email and the candidate eventually confirms (auto-set when the candidate's `pipeline_status` flips to `confirmed` within N days of the email send, or manual via the fill-a-cancellation page). Enables analytics like "we lost £X to cancellations this quarter, recovered £Y via fills" on the cancellation analytics dashboard above. Jon's view: "great ideas if we can track it cleanly. Not a biggie if not." Discussed May 2026 — captured for separate scoping.
...
- **Job Issues / Problems register — Stage 2 control panel (May 2026)** — Stage 1 (Apr 2026) lived on `job_requirements` with `requirement_type='issue'`. Stage 2 (May 2026, migration 075) **promoted storage to a dedicated `job_issues` table** with proper FK anchors and a structured event timeline. The public API stayed at `/api/problems/*` so callers didn't notice the storage swap. Migration auto-translated existing Stage 1 rows into the new table (preserving UUIDs), then soft-cancelled the originals.
  ...
  **Stage 4 (later):**
  ...
  - Retire / migrate the standalone `/vehicles/issues` route into `job_issues` (Stage 2 chose to coexist; Stage 4 unifies)
  - Cross-vehicle / cross-stock-item recurrence reporting ("RX22SXL had 3 clutch issues in 6 months")
  - **Returns & Completed close-out gate.** When a returned job has open issues, it can't auto-complete. Stage 2 doesn't enforce this — completion modal could be extended to warn ("1 open issue, complete anyway?"). Existing close-out warning pattern fits.
