...
- [x] **Slice 5 (Operations tab + cohesion):** `/operations/carnets` master/detail page (filterable list + rich `CarnetSection` cockpit, `?job=` auto-selects) + Operations nav. Job-View `carnet` requirement card is now a read-only reflection — generic dropdown + 6-step bar suppressed, replaced by a live lifecycle stepper + "Manage carnet in Operations →" link. `syncCarnetRequirementStatus` maps `job_carnets.status → carnet` requirement status on PATCH/cancel (keeps the prep counter honest). TODO: manual `client_arranges` create UI + dashboard NeedsAttention bucket.
