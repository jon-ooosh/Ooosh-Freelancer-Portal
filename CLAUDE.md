...
**Due dates — `resolveDueDate()` in `services/supplier-terms.ts` is THE definition (Sep 2026, migration 197).** The bill due date is DERIVED, not stored, and was being re-derived independently at five sites (the costs list, get-one, the bill push, the bill re-sync, and by extension the mark-paid modal via the list payload) — each carrying its own copy of the freelancer-vs-supplier branch. Every one of those now calls `resolveDueDate(cost, supplierTerms)` (or the terms-resolving `resolveDueDateForCost(cost)` for a single row), so OP and Xero cannot disagree about when a bill is due.
