...
- **Vehicle condition report PDF failure (C3, 22 Apr 2026)** — BookOutPage.tsx reports "PDF report: PDF generation failed" in its Operation Status log after submit. Native OP endpoint exists at `POST /api/vehicles/generate-pdf` (`backend/src/routes/vehicles.ts:1740`) and SHOULD be called (catch-all Netlify proxy at 2587 only fires if no native route matches). Not yet diagnosed — needs server log captured immediately after a reproducer: `sudo journalctl -u ooosh-portal --since "5 min ago" --no-pager | grep -i "vehicles/pdf\|Generate condition"`. Possible causes: pdf-lib embedJpg/embedPng throwing on a weird photo format, corrupt base64 in one of the photos, Express body-size limit (we bumped nginx to 10m but Express default is smaller), or a specific signature buffer. Non-blocking since the hire agreement PDF + email (different artefact) is working — the condition report is a book-out walkaround record for internal use.
