...
- [ ] Slice 2: public client request form (Jotform port) + signed Letter of Authorisation PDF (two-signature: Ooosh appoints agent + client accepts liability; Ooosh signatory from `system_settings`) + GMR seeding from crossings — needs the Ooosh signatory signature image
...
- [~] **Slice 2 (partial) — Letter of Authorisation PDF + signature storage:** `services/carnet-authority-pdf.ts` (pdf-lib, two-signature letter — Ooosh appoints agent + client accepts liability; logo via `fetchLogo`; Ooosh signature + name/role/address from `system_settings` category `carnets`, migration 141; client signature = signature line until the form lands). `POST /api/carnets/:id/generate-authority` builds it, stores in R2, sets `signed_authority_url`, surfaces on the job Files tab. "Generate Letter of Authorisation" button on the cockpit. **Settings → Carnet** section (`CarnetSettingsSection`) uploads the signature (→ R2 via `/files/upload`, key into `carnet_ooosh_signature_url`) + edits signatory name/role/address. **Still TODO:** the public client request form (Jotform port — collects details + client signature + seeds GMRs + auto-generates the final 2-sig PDF) + send-timing scheduler.
