...
- **HireHop error 370 = "this deposit has nothing left to refund."** A deposit fully APPLIED to an invoice cannot be refunded (`OWNER=0`) — retrying never works. Release the amount back off the invoice first (edit the deposit→invoice CHILD application down; the deposit's own cash-in figure is untouched), then refund. Job 15628, Sep 2026 — the Stripe refund succeeded and the HH leg 370'd, leaving a real £91.12 recorded nowhere in OP. **Never re-press Refund after a 370** — the guard checks against the original amount, not the HH balance, so it will issue a SECOND real Stripe refund. Full runbook: `docs/reference/MONEY-AND-EXCESS.md`.
- **⚠️ A NEGATIVE `paid` on `billing_payments_save.php` is SILENTLY CLAMPED TO ZERO** — HireHop returns `success: true` and creates a real £0.00 row (proven on job 16668, Sep 2026). To release money back off an invoice you must EDIT the existing application down (`id: <appId>, paid: <reduced>`). **Never treat a save response as proof the money moved — re-read the deposit and check the balance actually changed.**
- **A deposit's refundable balance is `owing` NEGATED** (`available = -owing`); `credit - paid` on the same kind=6 row is the independent cross-check. A deposit sitting at zero proves nothing about the sign, so never infer the convention from one.
...
- Money is pushed to Xero in **two steps** — the write, then `accounting/tasks.php`. **Always via `services/hh-xero-sync.ts` `syncSavedRowToXero`**, never inline: it checks the result, reads back the sync parameters HireHop names in its own save response, and returns `{ ok, error }`. Pass it the WHOLE save response, not just the row id.
- **A Xero refusal is NOT a reason to fail the operation, and NOT something to log and forget.** The money has usually already moved and HireHop is usually right — only Xero is short. Record it on the row's notes + a job timeline note + `sendXeroSyncFailedAlert` (admin-only; the remedy is always "fix it in Xero"), and surface it as an amber panel. Five paths used to `await` this call and discard the result, so a £120 refund reached Stripe and HireHop, was refused by Xero with a perfectly clear message, and told nobody (job 15187, Sep 2026).
- **Xero error 2263 ("Payments can only be made against Authorised documents") is not fixable from OP.** A credit note approved AFTER its deposit was allocated in Xero leaves HireHop and Xero mirror-imaged: HireHop's credit note is consumed and its deposit free; Xero's overpayment is PAID and its credit note unfunded. **Never post the refund against the credit note in HireHop to compensate** — it double-counts there and leaves the deposit reading as refundable, inviting a second real refund. Fix it in Xero by hand (cash refund against the credit note). Runbook: `docs/reference/MONEY-AND-EXCESS.md`.
- **`ACC_ID` on a `billing_list.php` row is the cloud icon in data form** — Xero object id once the row has landed, empty until then.
...
- **"Is it already in Xero?" is `xero_object_id` ALONE — never the sync state** (`existsInXero()`). `xero_object_id` says the object EXISTS; `xero_sync_state` says the last OPERATION succeeded. `recordError()` sets state `error` and leaves the id, so guards reading `id && state IN (pushed)` treated a live bill whose attach step 429'd as "not in Xero" and **created a duplicate**. Re-sync was worst: it fell through to the create path on any non-pushed state. Eleven live costs were primed when this was found (Sep 2026).
- **`xero_object_type` says which Xero entity `xero_object_id` is** (migration 209). Re-sync used to re-derive Invoice-vs-BankTransaction from the CURRENT `payment_method`, so changing the method after a push sent an Invoice id to the BankTransactions endpoint — the live "specified BankTransactionID does not match a known bank transaction" errors.
- **Paying many bills at once goes through `services/cost-batch-pay.ts` and creates a Xero BATCH PAYMENT**, never a loop of `payInvoice`. Twenty individual payments are each correct in the ledger but leave twenty rows to tick against one bank line — the tedium bulk pay exists to remove, moved into Xero. Two rules: **every bill is validated before anything is sent** (one ineligible bill refuses the whole batch BY NAME — a run that paid 19 of 20 with no record of the skip is worse than one that paid none), and **Xero first, then us** (if the batch is rejected nothing in OP has moved; the reverse order leaves bills marked paid with no money recorded anywhere). No silent fallback to individual payments — it would work, and quietly deliver the mess the user picked a batch to avoid.
- **`payment_status = 'paid'` + `xero_payment_id IS NULL` + a bill in Xero is an ARMED TRIGGER.** Three paths read exactly that shape as "this bill still needs its payment recording in Xero" (`pushBill` step 2, the background push fired by any cost edit, and the `/sync-xero` Retry button), so anything that marks a bill paid without a payment id must ALSO set `settled_externally` (migration 224) — otherwise the next person to edit the description pays the supplier a second time. Never add a fourth way to mark a bill paid without deciding which of the two it is.
- **A bill paid IN XERO comes back on its own** — `services/cost-xero-payment-sync.ts` (daily 07:50 + the "Check Xero for payments" button) asks Xero whether our outstanding bills are PAID and marks them paid here with Xero's real `PaymentID`. Don't hand-clear one that this would find; the real payment id is what disarms the trigger above, and a hand-cleared row never gets it. Part-paid and voided bills stay on the list on purpose.
- **`POST /costs/:id/settle-external` clears the OP ledger and NOTHING else.** It doesn't pay, void or tidy anything in Xero — if OP's own bill is still open there it stays open, which is why the modal warns and the note is mandatory. Don't "improve" it by voiding the Xero bill: void semantics per state are an open decision (see `ROADMAP.md`), same as for cost deletion.
- **A refund is a SECOND row, never an edit to the first.** The purchase happened and the receipt says so; the money coming back is its own event (migration 229). Editing the original down leaves a row that disagrees with its own evidence, VAT that doesn't tie, and no record of the refund at all — and it can't represent the two real bank lines.
- **A credit stores NEGATIVE money, and `applyCreditSign()` is the only thing that decides the sign.** Clients send positive magnitudes, as staff read them off the slip. Negative is what makes every existing `SUM(amount_gross)` — job actuals, vehicle spend, the by-job read — net to the true cost with no changes; a positive credit that slips through makes a refund ADD to what we spent, silently.
- **A credit inherits its purchase's facets** (`prepareCreditFromParent`) — job, vehicle, account code, cost type, card. It only nets where the spend landed if it is coded the same; a van part refunded as an overhead leaves the van overspent forever with nothing to flag it. Never re-pick them at the call site.
- **A credit pushes to Xero as a RECEIVE bank transaction with POSITIVE lines** (`asMagnitudes()` — the one sign flip, at the boundary). Watch the VAT: `resolveLineTaxType` reads `vat <= 0` as "no VAT", so pushing a credit's stored -£2.10 forfeits the reclaim without anything failing.
- **A credit against a BILL is a Xero credit note (ACCPAYCREDIT), which OP does not write.** It is recorded in OP with a calm advisory and raised in Xero by hand — never pushed down the bill path, which would raise a NEGATIVE ACCPAY bill, i.e. a second thing to pay.
- **Reducing a recharge after a refund is a human job, and so is re-splitting.** `prepareCreditFromParent` warns and bells the admins instead (HireHop silently clamps a negative to zero, so "just push a negative item" is not available). A credit is never itself a recharge — inheriting `recharge_mode` would put a refund on the Recharges tab asking to be billed to a client.
- **No cost lines on a credit** (v1). `validateCostLines` reconciles against a negative total, and splitting a partial refund back across the purchase's lines is real work for a rare case. The create route drops `lines` on a credit rather than half-answering it.
- **Deleting a cost in OP does NOT touch Xero.** `DELETE /costs/:id` is a hard delete (cascading `cost_lines` + `cost_allocations`); the Xero bill/transaction survives, orphaned. The capture list warns about this explicitly and refuses on a reconciled cost — keep both if you touch that path, and don't add a Xero void without deciding the per-state semantics first (see `docs/reference/ROADMAP.md`).
...
- **Today's date goes in the receipt extractor's USER message, never the system prompt.** The system prompt is byte-identical across calls so one `cache_control` breakpoint serves it at ~10% input cost; a date in there busts that cache daily. Without the anchor the model has no sense of "recent" and guesses a year — which is how fuel receipts arrived dated 2024.
- **An implausibly OLD cost date is flagged, never rewritten.** `normaliseCostDate` downgrades confidence past 120 days and the modal offers the same day/month in the most recent year as a one-click fix. A genuine historic invoice does get captured occasionally, so moving its date for it would be worse than the misread. (The future-date branch DOES repair, because a day↔month swap there is unambiguous.)
- **Supplier matching goes through `services/supplier-match.ts`** — learned alias → normalised exact → near match OFFERED for confirmation, never applied silently. Comparing lowercased names by substring saw no relation between "High Class-Cleaning LTD" and "High Class Cleaning" and offered to create a duplicate contact. A confirmed near match is remembered in `supplier_aliases`, so the question is asked once per letterhead.
