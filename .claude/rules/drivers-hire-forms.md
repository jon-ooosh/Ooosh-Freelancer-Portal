...
- **Validity is derived from DATES only — document presence is a separate question.** `services/driver-documents.ts` is THE answer to "is this document on file" (`drivers.files`, matched on a normalised tag/label token). It changes what staff are TOLD, never a window: the hire-form router is date-based too, so making a missing file invalidate a window would have OP and the driver's own form disagreeing about the same driver. `frontend/.../EvidenceGroup.tsx` `filesForSlot()` holds the same spellings for a different job (which thumbnail goes where) — **add a new spelling to both**.
- `frontend/src/lib/driverStatus.ts` `deriveDriverStatus()` mirrors the SQL CASE in `routes/drivers.ts`. **Keep the two in step** — the `/drivers` filter pills run through the SQL, so a mismatch means clicking "Expired" returns rows badged "Approved".
...
- **Only an explicit non-match trips review.** An absent face result is NOT a failure (a passport-only session runs no comparison).
...
- **Document matching is on a NORMALISED token** (lowercase, strip non-alphanumerics; tag first then label). Upload paths spell things `licence_front` / `license_front` / `Licence Front`, so exact-string matching silently drops images. `DOC_MATCH_TOKENS` is kept as an inline backend map deliberately — the backend doesn't import `shared/` at runtime.
