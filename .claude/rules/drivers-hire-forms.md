...
- **Passport = 90 days from the check date**, capped by the passport's own printed expiry — same window as the licence (migration 207). It was 30 while the hire-form app wrote `passportValidUntil = today + 90`, so `backfillFromDates` back-computed a check date 60 days AFTER the real one and staff read that as "Checked on" (Louis Salanson / 16507). The app now sends `passportCheckDate`, so nothing is back-computed. **Changing a `VALIDITY_WINDOW_DAYS` constant needs a migration** — the `*_valid_until` columns are STORED and only re-derive on the next write to that driver, so the constant alone leaves computed and stored surfaces disagreeing.
