...
- **To SHOW one, use `hooks/useAuthedFileUrl.ts` rather than calling `api.blob()` in an effect.** Every thumbnail needs the same three things and the hand-rolled copies kept missing them: wait until the element is near the viewport, abort if the caller loses interest, revoke the object URL on unmount. Rolled by hand per component it cost a 25-second Costs page — ~90 requests and ~100MB of full-size originals to draw a column of 32px icons. Attach the returned `ref` to the PLACEHOLDER as well as the loaded element; the ref is how the hook learns the thumbnail is on screen, so a ref only on the success branch never loads at all. Pass `enabled: false` when the caller already knows no image is coming (a PDF rendering as an icon) and nothing is requested. Fetching on a click into the SAME page — a lightbox, a download — is fine as a plain `api.blob()` call; the hook is for things that render on mount.
- **To OPEN one in a new tab, use `lib/openAuthedFile.ts` (`openAuthedFile` / `openR2Key`) — never `window.open()` after an `await`.** A browser only honours `window.open` while the click's transient user activation is live (~5s in Chrome, stricter in Safari), so `await api.blob(...)` then `window.open(...)` is silently eaten by the popup blocker whenever the file is big or the connection is slow — staff see nothing happen and report it as "the receipt won't open". It works on a fast connection, which is why thirteen hand-rolled copies of it survived. The helper claims the tab synchronously first, points it at the bytes when they land, degrades to a download if popups are blocked outright, and revokes the object URL. It throws what `api.blob` threw, so callers keep their own error copy.

## Time inputs

- **`step` on `<input type="time">` does NOT constrain the picker.** It drives
  validation and the spinner arrows only; Chrome's dropdown — the control people
  actually click — lists every minute regardless. A `step={900}` field will
  cheerfully take 09:07. If the granularity matters, own the options: see
  `components/QuarterHourSelect.tsx`. Snapping the value afterwards is not the
  answer — that silently changes what somebody picked.
- A `<select>` whose `value` matches no `<option>` renders BLANK, and then saves
  that blank over real data. Any select built from a fixed list must carry an
  out-of-list existing value as its own option.
