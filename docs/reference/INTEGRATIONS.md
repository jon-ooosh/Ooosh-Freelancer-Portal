...
- **Manual backstops** (`EmailProvenanceBar` in `ActivityTimeline.tsx`, `POST /api/auto-chase/emails/:id/{detach,move,hide,unhide}`): **detach/move** (accuracy, STAFF_ROLES — nulls/re-points `job_id`, audit in `reattached_*`); **hide/unhide** (confidentiality, **admin only** — drops the email off the all-staff timeline AND the AI reads but keeps it admin-visible; `hidden_at`/`hidden_by`). Timeline read excludes `hidden_at` for non-admins; `comms-summary` + `comms-query` exclude hidden ALWAYS.
