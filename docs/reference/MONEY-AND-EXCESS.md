...
By Sep 2026 that had accumulated to **£8,430.44 across 16 rows** on `/money/overview` — a card that reads as a liability and isn't one. Job 15855 is the shape: £432 taken, £108 kept against the invoice, **£324 refunded on 5 May**, and a £60 IOU still showing four months later.

**A SQL sweep understates it.** Querying `job_payments` for completed refunds finds only refunds *OP* recorded — 15855's £324 shows `already_refunded = 0` because that refund lives in HireHop and reaches the Money tab through `billing_list.php`, not through `job_payments`. Reconciling honestly means a HireHop read per job, which is why the overview (OP-cache only, no HH calls at page load) can't do it itself.

**What shipped instead of a reconciler:** the refund success panel lists any IOU still standing on the job and offers to clear it, at the one moment someone actually knows whether it is the same money. That stops the pile rebuilding; the existing backlog is a manual sweep with the Clear button.

- **Dismiss, never complete.** The refund already has its own completed row — marking the IOU complete as well would double-count it in every sum over completed refunds, which is the very thing making the headline untrustworthy.
- **`refunded_via_op`, not `refunded_externally`.** The money did go through OP, just not through this IOU; recording it as "outside OP" would be a small lie in an audit trail whose whole point is honesty.
- **Offered, never automatic.** Amounts often differ and OP cannot tell a part-payment from a duplicate. An IOU larger than the refund just made is flagged explicitly ("more than you just refunded — check the rest has been paid").
- Clearing is managers+ (as the existing Clear link is), but the IOU is *shown* to everyone — a staff refund must not dead-end in a 403.

`job_payments.id` is a **UUID**; `pending_refunds[].id` was typed `number` on both sides and only worked because JSON carried the real string through.
