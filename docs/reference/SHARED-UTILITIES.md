...
- **Multi-line subjects are flattened — `oneLineSubject()` (Sep 2026).** Resend returns a 422 ("The \\n is not allowed in the subject field") for any subject containing a line break, and that's a permanent failure — the email is dropped (logged `failed` + outage canary), never retried. Gmail SMTP had silently folded such subjects, so this was invisible until the Resend switch. Trigger: the RF21PWX damage issue had a multi-line summary; it broke the `vehicle_damage_logged` alert to info@ (19 Jul, subject `…{{summary}}`) and, when resolved, the `Issue resolved: <summary>` bell title that `notification-escalation.ts` emails as `URGENT: <title>` via `sendRaw` (24 Sep). `email-service.ts` now runs every subject through `oneLineSubject()` (line breaks → one space) in `send()`, `sendRaw()` and `renderPreview()`, before sending AND before `email_log`, so the log shows exactly what went out. **Keep it central** — patching individual callers would miss the next one (any typed text can reach a subject via a bell title). Multi-line summaries themselves are legitimate and deliberately NOT flattened at source. Same PR made the canary text provider-aware: on Resend it no longer says "check SMTP credentials / Google Workspace" (which pointed at a transport we no longer use).
...
- **Bundled-invoice allocation split** (`cost_allocations`, migration 092): split one cost across jobs via `PUT /costs/:id/allocations` — `CostAllocationModal`, surfaced both as a `⑂` row action AND a "Split across multiple jobs" tick at capture time (`onSavedAndSplit`). **Model = "Option A": one cost = one payable = one Xero bill; `cost_allocations` is a pure OP-side attribution layer** — a split NEVER changes what we owe the supplier or what's pushed to Xero, it only re-attributes the cost across jobs for OP cost tracking. Under-allocation is allowed (the modal shows an amber "£X unallocated (our cost) — OK to save" nudge; the remainder stays as our own cost, surfaced on no job); only OVER-allocation is blocked. When splitting a captured cost, the modal pre-seeds the capture job at the full amount as the first line, so the common "split the captured job's cost across others" flow is one add.
  ...
  - **Duplicate Xero bills — root cause and fix (Sep 2026, migrations 209).** A bill was edited in OP and a re-sync created a SECOND Xero bill (same reference 00148, one due 11 Sept and one due 3 Oct). Cause: every "already in Xero?" guard read `xero_object_id IS NOT NULL **AND** xero_sync_state IN ('bill_created','attached','reconciled')`, but `recordError()` sets state `error` and leaves the object id intact. So a cost holding a live, perfectly good Xero object whose last operation merely failed — a 429 on the attach step, say — read as "not in Xero", and the next push created a duplicate. `resyncCostToXeroLocked` was the worst offender: its first line fell through to `pushCostToXeroLocked` on ANY non-pushed state, so pressing "Re-sync to Xero" on an errored cost is literally what duplicated it; the PATCH route had an independent copy of the same mistake (`state === 'error' || state === 'pending'` → background push). **Eleven live costs were in that state when it was found** — 5 transient 429s, 4 reconciled/edited-away spend-money transactions, 2 wrong-entity errors. **Fix:** `existsInXero(cost)` keys on `xero_object_id` alone and every guard uses it. The conceptual point: `xero_object_id` answers *does it exist*, `xero_sync_state` answers *did the last operation succeed* — an errored cost is not an unsynced cost, it is a synced cost that is unhappy. **Second bug in the same family:** re-sync re-derived Invoice-vs-BankTransaction from the cost's CURRENT `payment_method`, so changing the method after a push pointed the update at the wrong Xero endpoint — the live "specified BankTransactionID does not match a known bank transaction" / "existing Invoice with the specified Type and InvoiceID could not be found" errors. `costs.xero_object_type` now records what was actually created (backfilled from the method, which is right for every cost whose method didn't change). **Repair path:** no SQL surgery — the red "Failed" pill's Retry (`POST /costs/:id/sync-xero`) now routes to the UPDATE path whenever an object exists, so re-syncing each errored cost either clears it or fails with an accurate reason. It still routes to `pushCostToXero` when a bill's payment leg is outstanding (object present, paid, no `xero_payment_id`), because that path's step 2 is what records the payment.
  - **Cost-date year anchoring (Sep 2026).** The new "Last year" period filter immediately surfaced receipts the extractor had dated 2025 and even 2024 — misread years, not old paperwork. Two fixes, one biasing and one checking. **Biasing:** the system prompt gained a THE YEAR rule (resolve an unclear/abbreviated/absent year to the most recent one that lands on or before today; never reach back), and **today's actual date goes in the USER message** — it cannot go in the system prompt, which is byte-identical across calls precisely so one `cache_control` breakpoint serves it at ~10% input cost. Without that anchor the model had nothing to date "recent" against. **Checking:** `normaliseCostDate` now downgrades confidence for anything older than 120 days (four months covers a year-end catch-up without nagging), and the capture modal shows an amber "that's over 4 months ago — are we really capturing a receipt from X?" with a one-click "use <same day/month, most recent year> instead". **It does NOT rewrite the year.** The future-date branch legitimately repairs, because a day↔month swap on an ambiguous date is unambiguous arithmetic; a too-old date is a judgement call, a genuine historic invoice does occasionally get captured, and silently moving its date would be worse than the misread it was trying to fix. Existing mis-dated rows are not touched — edit them by hand.
  - **Bulk pay via Xero batch payments (Sep 2026).** `services/cost-batch-pay.ts` + `POST /costs/pay-batch` (ADMIN_ONLY, same as `/:id/pay`) pay N bills with ONE Xero BatchPayment — the monthly garage run, twenty invoices and one bank transfer. **Why a batch rather than a loop over `payInvoice`:** twenty individual payments are each correct in the ledger, but the single bank line then needs matching against twenty rows in the bank feed — exactly the tedium bulk pay was built to remove, just relocated into Xero. `xeroBroker.createBatchPayment()` wraps `PUT /BatchPayments`. **Two ordering rules, both about never leaving a half-done payment run:** (1) *all-or-nothing on validation* — every selected bill is checked (pay-later method, not already paid, `xero_payment_id` empty, has a Xero INVOICE not a bank transaction, approved, positive, GBP) before anything is sent, and one failure refuses the batch naming the offender, because 19-of-20 with no record of the skip is harder to unpick than 0-of-20; (2) *Xero first, then us* — the batch is created before a single row is marked paid, so a rejection leaves OP untouched and retryable; the reverse order would leave bills marked paid with no money recorded anywhere, the failure hardest to notice. The DB write is one transaction; the **audit inserts run AFTER the commit, best-effort**, because inside it a failed log line would roll back bills whose money is already in Xero. Per-bill `xero_payment_id` comes from the batch's constituent payments where Xero returns them, falling back to the `BatchPaymentID`. There is deliberately **no fallback to individual payments** when a batch is refused. Frontend: checkboxes on the Bills-to-Pay tab for admins only, restricted to rows that pass the same eligibility test, a sticky bar with count + total (warning when the selection spans suppliers, since that's one bank line covering several), and a separate `BatchPayModal` — `PayModal` carries remittance advice and a person picker, which are per-payee concepts that don't survive a batch. **Selections are pruned when the filters change**, so a row scrolled out of view can't stay in the batch. Amounts are full-only: a total that differs from the sum has a cause (credit note, disputed line, short payment) and spreading the difference would put wrong numbers in the ledger quietly.
  - **Supplier matching (Sep 2026, migration 210).** `services/supplier-match.ts` is THE matcher: learned alias → normalised exact → near match offered for confirmation. The old inline `canonicaliseSupplier` lowercased and checked substring containment, which sees no relation between "High Class-Cleaning LTD" (letterhead) and "High Class Cleaning" (Xero) — the hyphen and the "LTD" break it in both directions — so OP offered to create a duplicate contact. `normaliseSupplierName()` lowercases, expands `&`, strips punctuation and drops company suffixes **as whole words** (so "Coastal" keeps its letters), giving `highclasscleaning` on both sides. It also searches Xero on the normalised term as well as the printed one, because handing Xero a hyphenated letterhead string can return nothing to match against at all. **A near match is never applied** — a wrong auto-merge files costs against the wrong supplier, which is worse than the duplicate it avoids; the modal asks "is that the same supplier?" and a yes writes a `supplier_aliases` row keyed on the normalised printed name, so the question is asked once per letterhead and never again. `POST /costs/supplier-alias` records the confirmation. No negative is stored: a different supplier with a similar name deserves the same question next time.
  - **Cost lines, the consumers (Sep 2026).** The Money tab's Actuals card buckets a split invoice **per line** (each line's own `xero_account_code`, falling back to the cost's), so a £250 freelancer bill that was really £190 fee + £60 fuel stops counting entirely as Freelancer. `/costs/by-job` returns each row's lines in one batched query — but **only on a whole-cost row**: a split-in row is a SHARE of another job's payable, so bucketing by its lines would overstate this job. **`crew_fronted` finally gives "Fronted expenses" an actuals bucket**, and it WINS over the line's category — that is what the quote means by the word (money the crew laid out and reclaims, whatever they spent it on), so a crew-fronted fuel line is Fronted, not Fuel. An unsplit cost still buckets by its header code exactly as before. **AI line extraction**: the receipt extractor may propose `lines[]`, but only where the document ITSELF itemises — narrative text like "Driver services, 3 days — £250" has nothing to read and must return an empty array, and printed items sharing a code AND a VAT rate are bundled into one line (three zero-rated travel items = one 325 line, not three). `reconcileLines()` is the gate and the rule is one-way: a proposal that doesn't reconcile to the document's own gross AND VAT within 1p is DISCARDED and confidence drops — never nudged into agreement, never allowed to move the totals. A missing split costs thirty seconds of typing; a plausible-but-wrong one reaches Xero and nobody notices. Proposed lines are tagged `source='ai'` and badged "✨ from receipt". **Derived allocations (spec §7) are NOT built and are blocked**: nothing in the UI sets a line's `job_id`, so every line inherits the cost's, every cost resolves to one job, and the derivation would no-op on every real cost — it needs a per-line job picker first.
  - **Cost lines (Sep 2026, migration 205).** `cost_lines` breaks one payable into N lines, each with its own `amount_gross`, `amount_vat`, `xero_account_code`, optional `job_id` and a `crew_fronted` flag; `source` records typed-vs-AI. NULL code/job means INHERIT the cost's, not "unknown". Full design + the decisions behind it: `docs/COST-LINES-SPEC.md`. **`amount_gross` is authoritative; VAT is DERIVED (amended 10 Sep 2026).** The gross is what we owe: staff-typed, never written from lines, and a set of lines that doesn't sum back to it within 1p is REJECTED rather than saved-and-flagged, because an unbalanced saved cost is an unflagged problem with no owner. VAT is not part of what we owe, it is an analysis of it — the first cut had staff type it on the header AND every line, which gave two figures that could disagree and a save that failed with "the gross balances but the VAT doesn't", a state with no good fix. Now a cost WITH lines takes `amount_vat`/`amount_net` from them via `headerVatFromLines()`, applied server-side on write so the header can't disagree with its own lines whatever a caller sends; one balance rule survives, and none of it applies to a cost with no lines. In the UI a line states an amount INC VAT plus a RATE (No VAT / 20% / 5% / Manual £) rather than two money boxes that never said whether the total included the VAT — the way Xero itself works, and it makes the header's own VAT toggle legible as the invoice-wide default a line may override. The rate is not stored: `amount_vat` is, and reopening infers the rate back from `(gross, vat)`, the same trick `inferVatMode` uses for the header. **Lines ride in the create/update payload**, not a separate endpoint: the create route fires `pushCostToXeroBackground` immediately after the INSERT, so a follow-up write would race it onto a one-line bill (a standalone `PUT /:id/lines` was written and then deleted for exactly this reason — one write path, no drift). **The VAT point, which is the real justification:** `resolveLineTaxType` DERIVES the rate as `round(vat/net*100)`, so a header spanning mixed rates gives a blend — the spec's own £325 example (£250 no-VAT labour, £60 fuel incl. £10 VAT, £15 zero-rated travel) implies 3%, `getPurchaseTaxType(3)` matches nothing, TaxType is omitted and Xero falls back to the account default of 20%, booking £54.17 against a true £10. Only a homogeneous line carries a real rate, so the function now takes `{amount_vat, amount_net}` and each line resolves its own. **Money is compared in whole pence, never floats** — `33.33*3 = 99.99000000000001`, so `Math.abs(sum − total) > 0.01` rejected an exactly-1p residue, the very case the tolerance exists for (found by `cost-lines.test.ts`, not by review). Sub-penny residue is folded onto the LARGEST line by `grossesWithResidue()` when BUILDING the Xero payload only — stored lines keep what staff typed. Guards: `reclaim_split` can't have lines (it pushes its own 3-line Exclusive structure); a cost with a hand-made `cost_allocations` split refuses lines until it's cleared; `xero_sync_state = 'reconciled'` locks them; editing lines on a pushed cost sets `xero_stale` like any other Xero-affecting field. **No lines = today's behaviour exactly** — no backfill, and none wanted.
  - **Expected-vs-actual by category (Sep 2026):** both cards in the panel's quoted-vs-actual strip now carry a make-up, so a variance says WHERE it came from, not just how big it is. Expected buckets come off the quote (`freelancer_fee_rounded`/`freelancer_fee`, `expected_fuel_cost`, `expenses_included`, `travel_cost`; admin fee deliberately excluded — it's a markup, never a real invoice). Actual buckets come off `xero_account_code`: 320 → Freelancer, 410 → Fuel, 325 → Transport, everything else → Other. `/costs/by-job` selects `c.*`, so the code was already on the wire. **"Fronted expenses" deliberately has NO actuals bucket** — fronting is a fact about *who paid*, not about *what was bought*, and nothing on a cost row records it (a freelancer's £40 parking receipt is coded 411 exactly like parking on the company card, and `supplier_name` is free text with no person link; `cost_type = 'freelancer_invoice'` is derived from category 320, so it is not a "supplier is a freelancer" flag either). Fronted spend therefore lands in Fuel / Transport / Other by what it was, and the Expected card's Fronted row carries a dotted underline + tooltip saying so. **Known blind spot this exposes:** whole-invoice coding — a £250 freelancer bill that was really £190 fee + £60 fuel counts entirely as Freelancer. That is the case `cost_lines` exists to fix; cost lines will NOT fix the who-fronted-it question without a flag of their own.
...
- **UI is opt-in and out of the way** — one quiet "＋ Add supporting documents" link under the receipt pane, expanding only when asked for or when docs already exist. A fuel receipt captured in twenty seconds must not have to walk past this.
- **The "add another" control is a labelled button, not a bare file input (Sep 2026).** The first cut used a raw `<input type="file">`; because we clear `e.target.value` after every pick (so the same file can be re-chosen), it permanently read "No file chosen" and gave no signal that a second document was possible — jon reported he couldn't see how to add one. It now sits inside a `<label>` with the input hidden, reading "＋ Attach a document" / "＋ Add another document", and the header states the remaining allowance ("2 of 9 · 7 more allowed"). The shared `ReceiptThumb` (`components/costs/CostReceipt.tsx`) carries a `+N` pip so the Costs hub and the Money tab both show that extra paperwork is filed. **Known gap:** a cost with supporting docs but NO main receipt renders no pip (the thumb early-returns on a missing `receipt_r2_key`) — an unlikely shape, and the capture modal still lists them.

**A saved cost must come back with its job identity — `withJobLabels()` in `routes/costs.ts` (Sep 2026).** `RETURNING *` gives the bare `costs` row, so `hh_job_number` / `job_name` / `vehicle_reg` — which live on the joined tables — were absent from the create and update responses. The frontend hands that row straight to the capture modal's job chip and to the split modal it can open next, both of which then had nothing to print and fell back to a bracketed `(linked job)` / `(captured job)`, so splitting a cost across several jobs showed a note where staff needed the number. Both responses now pass through `withJobLabels` (display sugar — it swallows its own errors and never fails a save), and the remaining placeholders name the job or say "no HireHop number" rather than a bare `(job)`. Same class as the `ExcessPaymentModal` merge-don't-replace rule. **Any new endpoint returning a cost row to the UI should do the same.**
...
- **Bill payment pull-back sync** (`services/cost-xero-payment-sync.ts`, daily 07:50 Europe/London, plus an admin "Check Xero for payments" button on Bills to Pay → `POST /costs/sync-xero-payments`): the same idea for BILLS. Polls Xero `Invoices` (chunked Guid-OR `where`, so `Payments`/`AmountDue`/`Status` come back in full) for OP bills still unpaid here, and where Xero says PAID marks them paid using Xero's own `PaymentID` and payment date. Writing the real `xero_payment_id` is the point: `payment_status='paid'` + a null `xero_payment_id` + a bill in Xero is exactly what three push paths read as "still needs its payment recording", so a bill paid in Xero and marked paid here any other way is primed to be paid a SECOND time by the next edit. Part-paid bills (`AmountDue > 0`) and voided/deleted ones are deliberately left on the list — the first is genuinely still owed, the second is a discrepancy that should stay visible. A bill Xero cleared with no Payment at all (credit note, prepayment) is flagged `settled_externally` instead, since there is no payment id to hold and there must never be a payment leg for it.
- **Settled outside OP** (`costs.settled_externally`, migration 224; `POST /costs/:id/settle-external`, ADMIN_ONLY, note required): the manual escape hatch for what the pull-back can't reach — no bill in Xero, settled by a route OP never saw, or the money landed on a duplicate bill someone keyed by hand. Clears the OP ledger and records WHY; never touches Xero. The flag is persisted rather than a one-off "skip the push" precisely because of the double-pay shape above — `pushBill` step 2, `recordBillPayment` and the `/sync-xero` retry all check it. It does NOT tidy Xero: if our own bill is still open there, the modal says so and someone has to void it in Xero.
- **Credits — money coming back** (`services/cost-credit.ts`, migration 229; `is_credit` + `refund_of_cost_id` on `costs`): a supplier refund is a SECOND cost row, negative and linked to the purchase, never an edit to it. The purchase keeps its receipt, its VAT and its figure; the two rows net in every existing total because a credit is literally a negative number. Sign rule: clients send POSITIVE magnitudes and `applyCreditSign()` stores them negative — the only place the sign is decided. `prepareCreditFromParent()` inherits the purchase's job/vehicle/account/card (a credit coded elsewhere leaves the original overspent forever), enforces the ceiling (existing credits count, so two £6 refunds of £12.60 are fine and a third isn't), refuses a credit of a credit, and forces `recharge_mode = 'none'` + `payment_status = 'paid'` — a credit is never payable and never itself a recharge. Xero: a card refund pushes as a **RECEIVE** bank transaction with positive lines (`asMagnitudes()`), so it lands on the card feed and reconciles through the existing 07:45 sync for free; a credit against a BILL is left for a human to raise as a credit note in Xero, with a calm advisory on the row, rather than pushed as a negative ACCPAY bill. Recharged or split purchases are warned about and belled to admins, never auto-adjusted — HireHop clamps a negative silently. UI: `RecordRefundModal` (the `↩` row action, or the "This is a refund" hand-off from Capture cost, which carries the slip over), the purchase row showing "−£x back", the credit row showing "↩ refund of …". Deliberately out of scope in v1: cost lines on a credit, and any automatic recharge adjustment.
- **Costs table UX:** click-to-sort headers (Date/Supplier/Description/Gross/Type/Status), truncated Type + Description with hover tooltips, icon Edit/Delete, Linked column links through to Job Detail / Vehicle Detail. **The row must fit the viewport without horizontal scroll (Sep 2026)** — at 10 columns the Actions cell was pushed off-screen entirely, so "Uploaded by" folds into the Supplier cell as a grey sub-line (it was a full column on the All costs tab) and the Xero pill rides inside the Status cell — both answer "where has this cost got to", and the Xero pill stays clickable for re-sync. **Adding a column means folding another one in, not widening the table.** The split-allocation line in the Linked cell stacks one job per line (capped at 3 + "+N more", full list in the tooltip) — rendered inline, a three-way split ran to ~250px and, because a column is as wide as its widest row, set the width of the whole column on its own. Mark-paid modal shows invoice number + due date (invoice date + 30, mirroring `addDaysISO` in the push) and defaults to `lloyds_transfer`.
