...
| **Sitter till (§5)** | Freelancer portal (the Next.js app in `src/`, Netlify): `/shift/[date]/till`, linked from the shift page. Price lookup any time the sitter can see the shift; selling only while the night is OPEN (its date, or until 06:00 next morning) **and the lock-up report hasn't been submitted** — locked up = till closed, test accounts included (jon, Sep 2026). Walk-in or tonight's bands (from the shift itself); tenders mirror the staff till (`services/shop-tenders.ts` — ⚠️ update it AND `frontend/src/lib/shopTenders.ts` when the Stripe terminal replaces Worldpay); list price only (freelancer cap 0%, and the route never takes a price from the phone). A sitter can cancel their own sale inside the hold; refunds are the office's. OP side: `/api/portal/studio-sitter/shifts/:date/till/*` behind the same rostered-to-this-evening gate as the lock-up; portal side: one whitelisted catch-all proxy. Sitters are `people`, so a sale records `recorded_by_person_id` + `shift_id` (migration 251; `recorded_by` → users is now optional, a CHECK keeps one set). |
| **Sitter review** | Every sitter sale is `needs_review`. Staff tick them off on the till's **Sitter sales** tab (`/money/shop?tab=review` — `?tab=` opens any tab). Linked from the staff lock-up report ("N to review →"), the handover-thread summary, **the lock-up email to info@ (an amber "To do: N shop sales from tonight to review" box) and the lock-up bell**. Reminder email after `shop_review_reminder_hours` (12) to `shop_review_reminder_to` (info@), once per sale. |
| **Checkout colours (both tills)** | Dark total bar; a CHOICE is an outlined ✓ (never a filled block); only the button that records the sale is green. Sitter till: *Who's it for* defaults to the band when exactly ONE is in, forces a pick when two are, walk-in when none; "Tonight" sits at the foot of the screen. |
| **Settings** | Settings → **Shop till** (admin only) lists every `shop` setting as an editable box; JSON values are validated before saving. |
| **Receipts (step 11)** | `services/shop-receipts.ts`. Emailed VAT receipt, email body (client layout — its footer carries our name, address and VAT number, so they live in ONE place). Per line: qty, each inc VAT, VAT rate, total; then ex-VAT, VAT, total paid; paid by; the OT-SHOP number and time of supply. **Refund receipt** for a refund, referring to the original sale's number. Never for "their bill" (nothing paid — their invoice is the document), consumption, or a cancelled sale. Staff till: optional box at checkout (the band's contacts offered via `job-contact-candidates.ts`), **Receipt** on any Recent Sales row (earlier addresses suggested), an optional box on the refund form (pre-filled with where the sale's receipt went). Every send logged in `shop_receipts` (migration 253). **Sitter till** (migration 254): an optional box at checkout (tonight's band's contacts offered as tap-to-pick buttons — jon, Sep 2026: sitters are entitled to reach the band they're looking after) and a *Receipt* button on tonight's own sales. **Both tills check the address BEFORE the sale is taken** (blank = no receipt), so a typo never leaves "sale recorded, but the receipt didn't send". Switching *who it's for* clears the address on both tills (jon: wrong-recipient risk). PDF receipts: parked (jon, Sep 2026 — the email is fine). |
