...
VE103B regen: if the original assignment had `ve103b_ref` set (international hire), the swap should trigger VE103B regeneration for the new van. The VE103B endpoint already exists (CLAUDE.md "Phase C5"). Backend extension: if `original.ve103b_ref` is set, fire VE103B generation for the new assignment in a `setImmediate` block post-swap. Manual cleanup of original cert: staff voids it from the VE103B page if needed (not auto-voided — see CLAUDE.md "VE103B certs are NOT voided on cancellation — requirements marked as not needed").
