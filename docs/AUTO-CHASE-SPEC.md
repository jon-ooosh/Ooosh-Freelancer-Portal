...
**Two manual backstops** on any ingested email, from the Activity Timeline (`EmailProvenanceBar` in `ActivityTimeline.tsx`; endpoints under `/api/auto-chase/emails/:id/*`):
- **Detach / move** (accuracy). "Not this job" nulls `job_id` (drops it off the timeline; RFC822 dedup stops re-ingestion re-creating it). `POST …/detach`; `POST …/move` re-points to a chosen job. **STAFF_ROLES** — a mis-attach is low-harm and recoverable. Audit via `reattached_from_job_id` / `reattached_at` / `reattached_by`.
- **Hide / unhide** (confidentiality). Drops an email off the all-staff timeline AND the AI reads (summary + dispute helper), but keeps it **visible to admin** — record preserved, not surfaced to everyone. `hidden_at` / `hidden_by`. `POST …/hide` / `…/unhide`, **admin only** (hiding conceals — it must not be a lever a staff member can pull on a complaint about themselves). The timeline read excludes `hidden_at IS NOT NULL` for non-admins; `comms-summary` + `comms-query` exclude hidden always (the AI must never read private content, even for admin).
