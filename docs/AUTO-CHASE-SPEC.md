...
**Remaining Phase 2 slices:** (a) wire `create-draft` into the 08:00 chase-due trigger for jobs with `auto_chase_mode='draft'` (the scheduler already scans due chases); (b) ChaseModal Off/Draft/Auto-send toggle (§9.4) persisting `auto_chase_mode`; (d) passive draft-vs-sent diff capture (§9.3); (e) "learn from this thread" timeline affordance that pre-fills the voice-tuning paste box from an ingested thread (small follow-on to the Settings paste box). Phase 4: the §7.2 dispute helper (NL query over the ingested chain) — the richer follow-on to the per-job summary — + line-item diff history.
