...
**Slice 3 SHIPPED — manual trigger + tuning surface.** "✨ Draft chase" button in `ChaseModal` (manager-tier) calls `create-draft` and shows recipient + threaded/standalone inline. `ChaseVoiceSettingsSection` on the Settings page edits `chase_voice_instructions` (no deploy). **Grounding fix:** `hireSpanDays` now gates the date phrasing — a single-day hire (span 0/1, absorbing HireHop's next-morning `job_end` rollover) is referred to as "your hire on <date>" instead of overstating a 2-day range (the reported 1-day-as-2-day bug). This is a code fix, NOT reachable via the voice setting.

**Remaining Phase 2 slices:** (a) wire `create-draft` into the 08:00 chase-due trigger for jobs with `auto_chase_mode='draft'` (the scheduler already scans due chases); (b) ChaseModal Off/Draft/Auto-send toggle (§9.4) persisting `auto_chase_mode`; (d) passive draft-vs-sent diff capture (§9.3); (e) Gmail search-for-thread so standalone chases latch onto the sent-quote thread (+ the cold-start backfill capstone).
