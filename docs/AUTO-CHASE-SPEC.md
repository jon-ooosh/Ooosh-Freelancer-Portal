...
**Remaining Phase 2 slices:** (a) wire `create-draft` into the 08:00 chase-due trigger for jobs with `auto_chase_mode='draft'` (the scheduler already scans due chases); (b) ChaseModal Off/Draft/Auto-send toggle (§9.4) persisting `auto_chase_mode`; (c) chase-voice Settings UI editing `chase_voice_instructions`; (d) passive draft-vs-sent diff capture (§9.3); (e) optional: Gmail search-for-thread so standalone chases latch onto the sent-quote thread.
