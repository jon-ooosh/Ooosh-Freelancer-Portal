...
### 5.3a Thread-anchoring + manual attribution controls (BUILT — Sep 2026, filtering foundation)

Live-feedback round after several months of use. Jon's two findings once ingestion had bedded in: (a) following emails in a thread drift to reference OTHER jobs (a past hire mentioned in passing, several upcoming/quoted jobs discussed on one chain) and the wrong quote/job gets attached; (b) going multi-mailbox (§6, incl. the director's own inbox) means some genuinely-client-facing threads carry candid/sensitive content that must NOT hit the all-staff timeline. The honest answer to both: a proactive deterministic filter PLUS a one-click human backstop — no classifier is perfect on free-text email. AI screening (§10a) is deliberately deferred until the deterministic layer + backstops are proven.

**Persisted match provenance.** The matcher already computes `method` + `confidence` per hit (§5.3) — these are now stored on the interaction (`interactions.match_method` / `match_confidence`, migration 217) instead of discarded. Drives the timeline provenance chip (a weak `sender_person` match shows "auto-attached · sender match"; a high-signal PDF/`#ref` match shows nothing) and thread-anchoring below.

**Thread-anchoring (the drift fix).** Once a thread's first ingested message strong-matches a job, later messages in that thread **inherit that anchor job** UNLESS they carry their OWN strong evidence for a *different* job — a `Quote (N)` PDF attachment, or an explicit job number in the **SUBJECT** line. A job number in the **BODY does NOT override the anchor** — bodies routinely reference other/past hires in passing ("like on #15804"), and that's exactly the drift. A weak (sender) or absent match in an anchored thread also inherits the anchor, so a bare client reply lands on the right job instead of the unmatched queue. Implemented in `gmail-ingestion.ts` (`getThreadAnchorJob` + the decision block); the backfill's `forceJobId` path is unchanged (it's already thread-gated by `threadBelongsToJob`). Stamped `match_method='thread_anchor'`. Note the quote-PDF version diff (§7.3) already routes each PDF by its own filename number independently, so a foreign quote never lands on the wrong version panel — thread-anchoring is about the email-body timeline attribution.

**Manual backstops** on any ingested email — all on ONE line under the email (`EmailProvenanceBar` in `ActivityTimeline.tsx`; endpoints under `/api/auto-chase/emails/:id/*`; the generic person/org/venue "Move" button is hidden for ingested emails in favour of "Move to job"):
- **Not this job** (detach, accuracy). A **tombstone** (migration 220, `detached_at` / `detached_by`): the email STAYS on the job as a greyed one-line "Removed from this job · Re-attach" row (like a system line) so a mistaken detach is recoverable, but is excluded from the AI reads (summary + dispute helper). `POST …/detach` / `…/reattach`. **STAFF_ROLES.** (The first cut nulled `job_id` and the email vanished — jon asked for the recoverable tombstone.)
- **Move to job** (accuracy). Search by **job number OR name** (`GET /hirehop/jobs?search=` already matches `CAST(hh_job_number AS TEXT)`) → `POST …/move` re-points `job_id` and clears any tombstone. STAFF_ROLES. Audit via `reattached_from_job_id` / `reattached_at` / `reattached_by`.
- **Hide / unhide** (confidentiality). Drops an email off the all-staff timeline AND the AI reads (summary + dispute helper), but keeps it **visible to admin** — record preserved, not surfaced to everyone. `hidden_at` / `hidden_by`. `POST …/hide` / `…/unhide`, **admin only** (hiding conceals — it must not be a lever a staff member can pull on a complaint about themselves). The timeline read excludes `hidden_at IS NOT NULL` for non-admins; `comms-summary` + `comms-query` exclude both hidden AND detached emails always (the AI must never read private content, even for admin, nor a wrong-job email).

**Design decisions (jon, Sep 2026):** client complaints CAN surface on the timeline (they're about the job) — the detach/hide backstops cover the rare "complaint about a specific staff member" case; hidden email stays admin-visible, not deleted; the attribution/hide controls + the (future) mailbox list all live behind the admin-only Settings gate.
...
### 13.4 Filtering foundation (BUILT — Sep 2026, PR 1 of the manager-mailbox rollout)
