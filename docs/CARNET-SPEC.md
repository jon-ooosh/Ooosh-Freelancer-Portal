...
- [ ] **Slice 5 (brought forward) — Operations > Carnets tab:** mount the rich `CarnetSection` management here (per-carnet), the global overview list, manual `client_arranges` create, + wire `job_carnets.status → carnet` requirement-card status so the Job-View tracker bar reflects real lifecycle progress. Dashboard NeedsAttention bucket.
