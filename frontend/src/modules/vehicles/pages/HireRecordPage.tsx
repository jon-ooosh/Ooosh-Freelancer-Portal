...
export function HireRecordPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Lightbox — zoom + pan to inspect detail */}
      {lightbox && (
        <PhotoLightbox key={lightbox.key} photo={lightbox} onClose={() => setLightbox(null)} />
      )}
    </div>
  )
}
...
/**
 * Full-screen photo viewer with zoom + pan. Scroll wheel / pinch to zoom past
 * fit-to-screen, drag (or one-finger) to pan, double-click/tap to toggle,
 * +/− buttons, and Download. Lets staff magnify into a specific dent rather
 * than just see the photo scaled to the viewport.
 */
function PhotoLightbox({ photo, onClose }: { photo: LightboxState; onClose: () => void }) {
    ...
    >
      ...
      <div className="absolute right-4 top-4 flex items-center gap-2" onClick={e => e.stopPropagation()}>
        ...
        <button type="button" onClick={() => applyScale(scale + 0.5)} title="Zoom in"
                className="h-8 w-8 rounded-full bg-white/90 text-lg font-bold text-gray-800 hover:bg-white">+</button>
        <button type="button" onClick={() => downloadPhoto(photo.key, photo.display, photo.filename)}
                className="rounded-full bg-white/90 px-3 py-1 text-sm font-medium text-ooosh-navy hover:bg-white">Download</button>
