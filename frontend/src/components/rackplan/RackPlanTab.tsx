...
export default function RackPlanTab({ jobId }: Props) {
  ...
  const actions: RackPlanActions = useMemo(() => ({
    ...
    setCapacity: (nodeId, capacity) =>
      updateNode(nodeId, (n) => ({ ...n, capacity_u: capacity })),
  ...
  const onEdgeDoubleClick = useCallback((_e: React.MouseEvent, edge: Edge) => {
    const label = window.prompt('Edit connection label', String(edge.label ?? ''));
  ...
  const addUItem = useCallback((item: ClassifiedRackItem) => {
    ...
    if (cap !== null && usedU + h > cap) {
      setHint(`Won't fit — ${target.data.node.label} is ${cap}U and already has ${usedU}U. Add a bigger rack or free up space.`);
      return;
    }
    setHint(null);
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap mb-2 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-400 hidden sm:inline">
            Drag a node's right dot → another node's left dot to add a labelled connection.
          </span>
      ...
      <div className="flex gap-3 flex-1 min-h-0">
        ...
        <div className="w-64 border border-gray-200 rounded-md overflow-y-auto p-2 space-y-3 bg-gray-50 shrink-0">
          <PickerSection title="Pre-built units" items={buckets.pre_built} placedIds={placedIds}
            onAdd={(it) => addStandaloneNode(it, 'pre_built')} />
          <PickerSection title="Cases" items={buckets.case} placedIds={placedIds}
            onAdd={(it) => addBuiltHereNode(it.name, it.rackHeight)}
            badge={(it) => (it.rackHeight ? `${it.rackHeight}U` : '')} />
          <PickerSection title="U-items" items={buckets.u_item} placedIds={placedIds}
            onAdd={addUItem} badge={(it) => `${it.rackHeight ?? '?'}U${it.halfWidth ? ' ½' : ''}`} />
          <PickerSection title="Loose" items={buckets.loose} placedIds={placedIds}
    ...
    <div>
      ...
      <div className="space-y-1">
        {items.map((it) => {
          ...
          return (
            <button key={it.itemId} disabled={placed} onClick={() => onAdd(it)}
              className={`w-full text-left text-xs px-2 py-1 rounded border flex items-center justify-between gap-1 ${
                placed ? 'bg-gray-100 border-gray-200 text-gray-400 cursor-default'
                  : 'bg-white border-gray-300 hover:border-ooosh-400 hover:bg-ooosh-50 text-gray-800'}`}>
              <span className="truncate leading-tight">{it.quantity > 1 ? `${it.quantity}× ` : ''}{it.name}</span>
              <span className="shrink-0 text-[10px] text-gray-400">{placed ? '✓' : badge ? badge(it) : ''}</span>
            </button>
          );
        })}
      </div>
