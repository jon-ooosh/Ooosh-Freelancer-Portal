...
const SYSTEM_PROMPT = `You answer an Ooosh Tours staff member's question about the email conversation with a client for a specific hire. You are given the full email chain for the job. Answer ONLY from these emails — this is used to settle "who asked for what, when" (disputes, "where's my X?", "did we confirm the dates?").

RULES:
- Ground every claim in the emails provided. Quote the relevant line(s) and give the DATE of the message. Never guess, infer beyond what's written, or invent.
- If the answer isn't in the emails, say so plainly (e.g. "I can't find anything about that in the emails on this job") — do not speculate.
- Lead with the direct answer, then the supporting quote + date. If several messages are relevant, walk through them in date order.
...
export async function answerCommsQuery(jobId: string, question: string): Promise<{ answer: string } | null> {
  ...
  const chain = await buildChainContext(jobId);
    ...
    messages: [
      {
        role: 'user',
        content: [
          // Cache the (large, stable) chain so repeated questions on the same
          // job in a session reuse it — only the question below varies.
